...
## 4. 协议 Handler 编写模板
    ...
    private void handleProcess(NetClient client, Object message) {
        XxxRequest req = (XxxRequest) message;
        
        // 1. 获取玩家
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        
        // 2. 基础校验（参数、前置条件）
        // 方式A：业务逻辑错误，推荐用 fail 抛出，由框架统一处理
        if (req.getCount() <= 0) {
        ...
        // 3. 获取模块（两种方式均可，优先使用 getXxxModule 如果存在）
        PetModule module = player.getPetModule();
        // 或者 PetModule module = player.getModule(PetModule.class);
