...
<script type="text/javascript">
    ...
    function pay(){
        ...
        wx.chooseWXPay({
            timestamp: <%=request.getAttribute("timestamp")%>, // 支付签名时间戳，注意微信 jssdk 中的所有使用 timestamp 字段均为小写。但最新版的支付后台生成签名使用的 timeStamp 字段名需大写其中的 S 字符
            nonceStr: '<%=request.getAttribute("nonceStr")%>', // 支付签名随机串，不长于 32 位
            package: 'prepay_id=<%=request.getAttribute("prepayId")%>', // 统一支付接口返回的prepay_id参数值，提交格式如：prepay_id=\*\*\*）
...
<p>金额：<%=request.getAttribute("price")%></p>

<p>物品：<%=request.getAttribute("itemName")%></p>
<p></p>
<p></p>
