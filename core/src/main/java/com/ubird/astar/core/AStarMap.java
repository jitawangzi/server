...
//	private Set< AStarNode> closeSet = new HashSet< AStarNode>();
	...
	public AStarMap(int[][] astarData, AStarNode source, AStarNode target) {
		this.astarData = astarData;
		this.source = source;
		this.target = target;
	}
	public AStarMap(int[][] astarData) {
		this.astarData = astarData;
	}
...
//						)
	...
	public boolean isBarrier(AStarNode node) {
		return astarData[node.getY()][node.getX()] == STATE_BARRIER || astarData[node.getY()][node.getX()] == 1;
	}
...
//		return closeSet.contains(node);
	...
	public AStarNode[] getNeighbor(AStarNode current) {
