...
public class VirtualServerRegistry implements Closeable {
	...
	private VirtualServerRegistry(CuratorFramework externalClient, String basePath) {
		if (externalClient == null)
			throw new IllegalArgumentException("CuratorFramework cannot be null");
	...
	private static String normalizeBasePath(String p) {
		if (p == null || p.isEmpty())
			return "/server/login/game/virtual-servers";
	...
	public List<String> initMissingFromViews(List<VirtualServerView> views) throws Exception {
		...
		for (VirtualServerView v : views) {
			...
			try {
				client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath(path, toBytes(v));
				created.add(v.ID);
	...
	public boolean updateOpenTime(String serverId, LocalDateTime openTime, int expectedVersion) throws Exception {
		return updateField(serverId, expectedVersion, v -> v.openTime = openTime == null ? null : openTime);
	}
	...
	public enum EventType {
		NODE_CREATED, NODE_CHANGED, NODE_DELETED
	}
	...
	public void startListening(ChangeListener listener) {
		...
		CuratorCacheListener l = CuratorCacheListener.builder().forCreates(cd -> {
			VirtualServerView v = safeParse(cd.getData());
			if (v != null && v.ID != null) {
				cacheMap.put(v.ID, v);
			}
			if (listener != null)
				listener.onEvent(EventType.NODE_CREATED, cd.getPath(), cd.getData());
		}).forChanges((od, nd) -> {
			...
			if (listener != null)
				listener.onEvent(EventType.NODE_DELETED, cd.getPath(), null);
		}).build();
