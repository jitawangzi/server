...
public class WebSocketDecoder extends CumulativeProtocolDecoder {
	...
	protected boolean doDecode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
		...
		if (!session.containsAttribute(WebSocketUtils.SessionAttribute)) {
			...
			if (tryWebSockeHandShake(session, in, out)) {
				...
				// handler.
				if (session.getAttribute("webRemaning") != null && session.getAttribute("webRemaning").equals(Boolean.TRUE)) {
					return false;
				}
	...
	private boolean tryWebSockeHandShake(IoSession session, IoBuffer in, ProtocolDecoderOutput out) {
		...
		try {
			...
			boolean isWeb = WebSocketUtils.isWebSocket(payLoadMsg);
			...
			if (socketKey == null || socketKey.length() <= 0) {
				session.setAttribute("webRemaning", true);
				return true;
			}
			...
			session.setAttribute("webRemaning", false);
