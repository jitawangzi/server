...
public class GemHandler extends GameBaseHandler {
    ...
    private void xiLian(NetClient client, Object message) {
        GemXiLianRequest_10000010 req = (GemXiLianRequest_10000010) message;
        String uids = req.getUids();
        GemXiLianResponse_10000011 defaultInstance = GemXiLianResponse_10000011.getDefaultInstance();
        GemXiLianResponse_10000011.Builder resp = GemXiLianResponse_10000011.newBuilder();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        GemModule gemModule = player.getModule(GemModule.class);
        Gem gem = gemModule.gemXiLian(Long.parseLong(uids));
        if (gem == null) {
            // 宝石不存在
