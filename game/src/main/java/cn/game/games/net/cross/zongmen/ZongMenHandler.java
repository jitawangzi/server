...
public class ZongMenHandler extends BaseHandler {
	...
	private void updateContributeValue(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist,
	...
	private void zongMenChat(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		ZongMenInfo zongMenInfo = ZongMenManager.getInstance().getZongMenInfo(zongMenId);
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.ChatResponse_31000002);
			return;
		}
	...
	private void findZongMen(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		ZongMenInfo zongMenInfo = ZongMenManager.getInstance().getZongMenInfo(zongMenId);
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.findZongMenResponse_40000004);
			return;
		}
	...
	private void ZongMenBuyShop(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		ZongMenInfo zongMenInfo = ZongMenManager.getInstance().getZongMenInfo(zongMenId);
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.ZongMenBuyShopResponse_40000048);
			return;
		}
		ZongMenMember member = zongMenInfo.getMember(playerId);
		if (member == null) {
		...
		if (config == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.config_data_not_found, PbProtocol.ZongMenBuyShopResponse_40000048);
			return;
		}
		if (playerLv < config.LevelUnlock) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.level_not_enough, PbProtocol.ZongMenBuyShopResponse_40000048);
			return;
		}
		...
		if (itemConfig.ShopItemQuota != 0 && member.buyShopItemNumMap.getOrDefault(itemConfig.ID, 0) + count > itemConfig.ShopItemQuota) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.shop_item_buy_count_max,
					PbProtocol.ZongMenBuyShopResponse_40000048);
			return;
		}
		if (!zongMenInfo.isEnoughAsset(itemConfig.PurchaseParameter, count, member)) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.resource_not_enough, PbProtocol.ZongMenBuyShopResponse_40000048);
			return;
		}
	...
	private void ZongMenActiveReward(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		for (int index : req.getIndexListList()) {
			if (member.getRewardLivenessIndexList().contains(index)) {
				sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_active_reward_already_get,
						PbProtocol.ZongMenActiveRewardResponse_40000046);
				return;
			}
	...
	private void updateMemberAuth(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.updateZongMenAssetResponse_40000038);
			return;
		}
		ZongMenMember member = zongMenInfo.getMember(playerId);
		...
		req.getTargetPidListList().forEach(pid -> {
			targetPidList.add(pid.longValue());
		});
		// 加入审批检测
		if (req.getOptType() == 1 || req.getOptType() == 2) {
			if (!permissionsConfig.Approval) {
				sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_permission_not_enough,
						PbProtocol.updateMemberAuthResponse_40000042);
				return;
			}
			for (long targetPid : req.getTargetPidListList()) {
				if (!zongMenInfo.hasApply(targetPid)) {
					sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.request_parameter_error,
							PbProtocol.updateMemberAuthResponse_40000042);
					return;
				}

				if (req.getOptType() == 1 && req.getTargetPidListList().size() == 1
						&& ZongMenHelper.isHasZongMen(zongMenId, req.getTargetPidList(0))) {
					sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_player_apply_has,
							PbProtocol.updateMemberAuthResponse_40000042);
					return;
				}
			}
			if (req.getOptType() == 1 && zongMenInfo.isFull()) {
				sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_full, PbProtocol.updateMemberAuthResponse_40000042);
			}
		}
		// 踢人 审批
		if (req.getOptType() == 3) {
	...
	private void updateZongMenAsset(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		ZongMenMsg.updateZongMenAssetResponse_40000038.Builder res = ZongMenMsg.updateZongMenAssetResponse_40000038.newBuilder();
		ZongMenInfo zongMenInfo = ZongMenManager.getInstance().getZongMenInfo(zongMenId);
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.updateZongMenAssetResponse_40000038);
			return;
	...
	private void quitZongMen(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		String playerName = paramList.get(0);
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.quitZongMenResponse_40000018);
			return;
		}
		ZongMenMember member = zongMenInfo.getMember(playerId);
		// 对宗主的处理
	...
	private void setZongMenMemberPosition(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		ZongMenMsg.setZongMenMemberPositionResponse_40000016.Builder res = ZongMenMsg.setZongMenMemberPositionResponse_40000016
				.newBuilder();
		ZongMenInfo zongMenInfo = ZongMenManager.getInstance().getZongMenInfo(zongMenId);
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist,
		...
		if (targetPositionNum >= targetPermissionsConfig.Number && req.getPosition() != ZongMenConstants.ZONG_MEN_POSITION_ZONG_ZHU) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_position_member_num_not_enough,
					PbProtocol.setZongMenMemberPositionResponse_40000016);
			return;
		}
		...
		sendMsgToGameServer(playerId, client, res.setResult(true).setPosition(req.getPosition()).setTargetPid(req.getTargetPid()).build(),
				PbProtocol.setZongMenMemberPositionResponse_40000016);
	}
	...
	private void getZongMenLog(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		ZongMenInfo zongMenInfo = ZongMenManager.getInstance().getZongMenInfo(zongMenId);
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.getZongMenLogResponse_40000026);
			return;
		}
	...
	private void setZongMenSetting(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		String playerName = paramList.get(0);
		if (zongMenInfo == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.dissolveZongMenResponse_40000012);
			return;
		}
		ZongMenMember member = zongMenInfo.getMember(playerId);
		GuildPermissionsConfig permissionsConfig = GuildPermissionsManager.instance().get(member.position);
//
		...
		if (!StringUtils.isEmpty(req.getName())) {
			...
			RedisLocalCache.getInstance().getAsync(CacheType.ZONG_MEN_NAME_ID.key(req.getName())).onSuccess((result) -> {
				if (result == null) {// 该名称 未被占用
					boolean redisLock = LockUtil.tryLockNoWaitSync(6, CacheType.ZONG_MEN_NAME_CHANGE_LOCK.key(req.getName()));
					if (redisLock) {
						// 删除旧的宗门 名称 id 映射
						...
						// 保存新的宗门 名称 id 映射
						ZongMenManager.getInstance().saveRedisNameIdMap(req.getName(), zongMenInfo.getId());
						sendMsgToGameServer(playerId, client, res.build(), PbProtocol.setZongMenSettingResponse_40000014);

					} else {
						sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_name_repeat,
								PbProtocol.setZongMenSettingResponse_40000014);
						return;
					}
				} else {// 该名称 被占用
...
//    - 宣言修改：玩家昵称修改了宣言；
		...
		if (req.getIcon() != 0 && zongMenInfo.getModule().setting.unlockIconMap.containsKey(req.getIcon())) {
	...
	private void applyJoinZongMen(long playerId, Message message, List<String> paramList, NetClient client) {
		...
		if (zongMenInfo.isFull()) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_full, PbProtocol.applyJoinZongMenResponse_40000008);
			return;
		}
		if (zongMenInfo.hasApply(playerId)) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_apply_exist, PbProtocol.applyJoinZongMenResponse_40000008);
			return;
		}
		// 开启自动加入 则直接加入宗门
		if (zongMenInfo.isAutoJoin()) {
			...
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_allow_join,
					PbProtocol.applyJoinZongMenResponse_40000008);
			return;
		}
	...
	private void createZongMen(long playerId, Message message, List<String> paramList, NetClient client) {
		...
		// 该名称被其他节点使用
		if (!createLock) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_name_repeat, PbProtocol.createZongMenResponse_40000006);
			return;
		}
		ZongMenManager.getInstance().createZongMen(req, name, playerId, createPlayerName, power, serverId).onSuccess(zongMenInfo -> {
	...
	private void getZongMenInfo(long zongMenId, long playerId, Message message, List<String> paramList, NetClient client) {
		...
		if (zongMenInfo == null || zongMenInfo.getMember(playerId) == null) {
			sendErrorCodeMsgToGameServer(playerId, client, ErrorMsgEnum.zong_men_not_exist, PbProtocol.getZongMenInfoResponse_40000022);
			return;
		}
