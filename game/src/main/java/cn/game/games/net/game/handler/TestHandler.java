...
//    	ServerContext.getInstance().fireEvent(ServerEventTypeEnum.NewDay, null);
		...
		// RFuture<Object> async =
		// RedisUtil.getAsync(CacheType.PLAYER_SIMPLE.key(240202120));
		...
		// }
		// });
		// Future<ItemModule> requestRemoteServer =
		// VxHolder.requestRemoteServer("game_test", new
		// ObjectProtocol(PbProtocol.ServerObjectTestRequest_7d000033, itemModule));
		...
		// });
		// Future<Long> future =
		// GameServer.getInstance().getRemoteLoginServerInterface(CallType.LoadBalancer).getUid2("323323");
		// future.onComplete(r -> {
		...
		// player.getQuestModule().addConditionCount(ConditionTypeEnum.ChapterFinish, 3,
		// 1, 2);
		// System.exit(0);
	}
	...
	private void drawTest(Player player) {
		...
		for (int j = 0; j < lp; j++) {
			for (int i = 0; i < count; i++) {
				...
				// if (i % 10 == 0) {
				// System.out.println(MessageFormat.format("第{0}次十连抽结果,紫:{1} 金:{2} 红:{3}", i /
				// 10, r4, r5, r6));
				// }
			}
		}
	...
	protected void testDBinsert(NetClient client, Object message) {
		...
		// TestDbInsertRequest_7f000001 request = (TestDbInsertRequest_7f000001)
		// message;
	...
	private void configCheck(NetClient client, Object message) {
		...
		for (RandomGivenConfig randomGivenConfig : list) {
			try {
				...
				// throw new IllegalAccessError("randomGivenConfig id 添加失败： " +
				// randomGivenConfig.ID);
			}
		}
		...
		// Iterator<Entry<Integer, List<BattleConfig>>> iterator =
		// battleTypes.entrySet().iterator();
		// while (iterator.hasNext()) {
		// int battleId = 0;
		// BattleConfig battleConfig = null;
		// Map.Entry<java.lang.Integer,
		// java.util.List<cn.game.protocol.generated.config.BattleConfig>> entry =
		// (Map.Entry<java.lang.Integer,
		// java.util.List<cn.game.protocol.generated.config.BattleConfig>>) iterator
		// .next();
		...
		// }
		//
		// cn.game.protocol.protobuf.BattleMsg.BattleFieldStartRequest_13000001.Builder
		// builder =
		// cn.game.protocol.protobuf.BattleMsg.BattleFieldStartRequest_13000001
		// .newBuilder();
