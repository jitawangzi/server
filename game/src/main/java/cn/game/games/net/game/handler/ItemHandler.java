...
public class ItemHandler extends BaseHandler {
	...
	protected void inititialize() {
		...
		putInvoker(PbProtocol.EquipLockRequest_0b000007, this::lock);
	}
	...
	private void sale(NetClient client, Object message) {
		...
		ItemMsg.DeprecatedItemSaleResponse_0b000002 resp = ItemMsg.DeprecatedItemSaleResponse_0b000002.getDefaultInstance();
		int id = req.getId();
		int num = req.getNum();
...
//		itemModule.delRepositoryItem(id, num);
	...
	private void lock(NetClient client, Object message) {
		ItemMsg.EquipLockRequest_0b000007 req = (ItemMsg.EquipLockRequest_0b000007) message;
		ItemMsg.EquipLockResponse_0b000008 resp = ItemMsg.EquipLockResponse_0b000008.getDefaultInstance();
		String uid = req.getUid();
		int type = req.getType();
		/*if (type == GoodsTypeEnum..getId()) {
			ChipOp chipOp = PlayerCacheFactory.getCache(client.getPlayerId(), ChipOp.class);
			if (!chipOp.changeLockState(uid)) {
				client.sendProtocol(resp, ErrorMsgEnum.data_not_found.getId());
				return;
			}
		} else*/
		client.sendProtocol(resp);
	...
	protected void use(NetClient client, Object message) {
		...
		ItemMsg.DeprecatedItemUseResponse_0b000004.Builder resp = ItemMsg.DeprecatedItemUseResponse_0b000004.newBuilder();
