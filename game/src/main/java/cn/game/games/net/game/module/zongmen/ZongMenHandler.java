...
import cn.game.protocol.protobuf.ZongMenMsg.ZongMenDonateRequest_40000067;
...
public class ZongMenHandler extends GameBaseHandler {
    ...
    static Logger log = LoggerFactory.getLogger(ZongMenHandler.class);

    public static Future<ZongMenCallbackMsg> sendMsgToZongMenServer(long zongMenId, Player player, Message req, String... params) {
        // 封装宗门请求
        ZongMenMsgRequest_41000045.Builder serverReq = ZongMenMsgRequest_41000045.newBuilder();
        // 设置请求参数
        // 宗门ids
        ...
        rpcFuture.onSuccess(result -> {
            if (result != null) {
                log.info(String.format("sendMsgToZongMenServerCallBack msgId:%d %s, errorCode:%d, pid:%d ", result.getMsgId(), req.getClass().getSimpleName(), result.getErrorCode(), result.getPlayerId()));
    ...
    protected void inititialize() {
        ...
        putInvoker(PbProtocol.ZongMenBountyBattleStartRequest_40000076, this::bountyBattleStart);
        ...
        putInvoker(PbProtocol.ZongMenDonateRequest_40000067, this::donate);
    }
    ...
    private void quickJoinZongMen(NetClient client, Object o) {
        ...
        ZongMenServiceInfo joined = null;
        ...
        for (ZongmenServiceInterface zongmenServiceInterface : allServerInterface) {
            ZongMenServiceInfo randomJoin = zongmenServiceInterface.randomJoin(player.getPlayerId());
            if (randomJoin != null) {
                joined = randomJoin;
                break;
            }
    ...
    private void bargain(NetClient client, Object o) {
        ...
        int bargainCount = zongmenModule.getBargainCount(); 
        if (bargainCount >= GlobalConst.ZongmenBargainMax) {
        ...
        int count = zongmenProxy.bargain(player.getZongMenId(), player.getPlayerId()); 
    ...
    private void buyBargain(NetClient client, Object o) {
        ...
        PlayerHelper.delResources(player, guildBargainConfig.Price[0], bargainPrice, OpType.ZongMenBargain);
    ...
    private void rewardLiveness(NetClient client, Object o) {
        ...
        ZongMenMsg.ZongMenActiveRewardResponse_40000046.Builder res = ZongMenMsg.ZongMenActiveRewardResponse_40000046.newBuilder();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
    ...
    private void updateMemberAuth(NetClient client, Object o) {
        ...
        sendMsgToZongMenServer(player, req, player.getPlayerName()).onSuccess(callBack -> {
    ...
    private void quitZongMen(NetClient client, Object o) {
        ...
        ZongMenMsg.quitZongMenResponse_40000018.Builder res = ZongMenMsg.quitZongMenResponse_40000018.newBuilder();
        autoForwardZongMenServer(client, res, req, (result) -> {
            // 退出成功
            Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
            player.getZongmenModule().clearZongMen();
            player.getZongmenModule().setDisbandCount(player.getZongmenModule().getDisbandCount() + 1);
            // 第二次及后续退出时，宗主需要1小时才可加入其它宗门（ZongmenSuzerainCD）；
            if (player.getZongmenModule().getDisbandCount() > 1) {
                player.getZongmenModule().setNextJoinTimer(System.currentTimeMillis() + GlobalConst.ZongmenMemberCD * DateUtil.HOUR_MILLIS);
            }
    ...
    private void setZongMenMemberPosition(NetClient client, Object o) {
        ZongMenMsg.setZongMenMemberPositionRequest_40000015 req = (ZongMenMsg.setZongMenMemberPositionRequest_40000015) o;
        ZongMenMsg.setZongMenMemberPositionResponse_40000016.Builder res = ZongMenMsg.setZongMenMemberPositionResponse_40000016.newBuilder();
        autoForwardZongMenServer(client, res, req, null);
    }

    private void setZongMenSetting(NetClient client, Object o) {
        ZongMenMsg.setZongMenSettingRequest_40000013 req = (ZongMenMsg.setZongMenSettingRequest_40000013) o;
        ZongMenMsg.setZongMenSettingResponse_40000014.Builder res = ZongMenMsg.setZongMenSettingResponse_40000014.newBuilder();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        ...
        sendMsgToZongMenServer(player, req, player.getZongMenName()).onSuccess(callBack -> {
            if (callBack.errorCode != ErrorMsgEnum.ok.ID) {
                ...
                if (module.getDisbandCount() > 1) {
                    module.setNextJoinTimer(System.currentTimeMillis() + GlobalConst.ZongmenSuzerainCD * DateUtil.HOUR_MILLIS);
                }
                client.sendProtocol(callBack.response);
            }
        }).onFailure(err -> {
    ...
    private void applyJoinZongMen(NetClient client, Object o) {
        ...
        ZongMenServiceInfo zongmen = serviceInterface.applyJoinZongmen(id, player.getPlayerId());
        if (zongmen != null) {
    ...
    private void createZongMen(NetClient client, Object o) {
        ...
        List<String> checkStrs = new ArrayList<>();
        if (!player.isEnough(GlobalConst.ZongmenCreationConsume[0], GlobalConst.ZongmenCreationConsume[1])) {
            client.sendProtocol(res.build(), ErrorMsgEnum.resource_not_enough.ID);
            return;
        }
        ...
        CompletableFuture.allOf(checkComplatableList.toArray(new CompletableFuture[0])).thenAcceptAsync(result -> {
    		...
    		ZongmenServiceInterface zongmenProxy = GameServer.getInstance().getZongmenProxy(0); 
    		Future<ZongMenServiceInfo> zongmen = zongmenProxy.createZongmen(player.getPlayerId(), req.getName(), req.getNotice(), req.getDeclaration(), req.getIcon()); 
    		zongmen.map(r -> {
                // 创建宗门成功
    ...
    private void zongMenList(NetClient client, Object o) {
        ...
        ZongMenMsg.getZongMenListResponse_40000002.Builder res = ZongMenMsg.getZongMenListResponse_40000002.newBuilder();
        ...
        // 查询指定页码的战斗力宗门数据
        CompletionStage<List<RankEntry>> zongMenListStage = RankService.getInstance().getTopNAsync(player.getServerId(), rankType, 30);
        // 获取宗门 simpleZongMen 列表
        zongMenListStage.thenCombine(rankSizeStage, (zongMenRankList, rankSize) -> {
            List<Long> zongMenIdList = zongMenRankList.stream().map(RankEntry::getPlayerId).collect(Collectors.toList());
    ...
    private void updateAsset(NetClient client, Object message) {
        ...
        updateZongMenAssetResponse_40000038 defaultInstance = updateZongMenAssetResponse_40000038.getDefaultInstance();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        client.sendProtocol(defaultInstance);
    }

    private void bountyAccept(NetClient client, Object message) {
        ZongMenBountyAcceptRequest_40000070 req = (ZongMenBountyAcceptRequest_40000070) message;
        ZongMenBountyAcceptResponse_40000071 defaultInstance = ZongMenBountyAcceptResponse_40000071.getDefaultInstance();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        client.sendProtocol(defaultInstance);
    }

    private void bountyTargetRefresh(NetClient client, Object message) {
        ZongMenBountyTargetRefreshRequest_40000074 req = (ZongMenBountyTargetRefreshRequest_40000074) message;
        ZongMenBountyTargetRefreshResponse_40000075 defaultInstance = ZongMenBountyTargetRefreshResponse_40000075.getDefaultInstance();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        ZongMenBountyTargetRefreshResponse_40000075.Builder resp = ZongMenBountyTargetRefreshResponse_40000075.newBuilder();
        client.sendProtocol(resp.build());
    }

    private void bountyBattleStart(NetClient client, Object message) {
        ZongMenBountyBattleStartRequest_40000076 req = (ZongMenBountyBattleStartRequest_40000076) message;
        String playerId = req.getPlayerId();
        ZongMenBountyBattleStartResponse_40000077 defaultInstance = ZongMenBountyBattleStartResponse_40000077.getDefaultInstance();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        client.sendProtocol(defaultInstance);
    }

    private void bountyBattleEnd(NetClient client, Object message) {
        ZongMenBountyBattleEndRequest_40000078 req = (ZongMenBountyBattleEndRequest_40000078) message;
        String playerId = req.getPlayerId();
        boolean isWin = req.getIsWin();
        ZongMenBountyBattleEndResponse_40000079 defaultInstance = ZongMenBountyBattleEndResponse_40000079.getDefaultInstance();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        client.sendProtocol(defaultInstance);
    }

    private void bountyBattleReport(NetClient client, Object message) {
        ZongMenBountyBattleReportRequest_4000007a req = (ZongMenBountyBattleReportRequest_4000007a) message;
        boolean isZongMen = req.getIsZongMen();
        ZongMenBountyBattleReportResponse_4000007b defaultInstance = ZongMenBountyBattleReportResponse_4000007b.getDefaultInstance();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        ZongMenBountyBattleReportResponse_4000007b.Builder resp = ZongMenBountyBattleReportResponse_4000007b.newBuilder();
        client.sendProtocol(resp.build());
    }

    private void bountyPlayer(NetClient client, Object message) {
        ZongMenBountyPlayerRequest_4000007c req = (ZongMenBountyPlayerRequest_4000007c) message;
        String playerId = req.getPlayerId();
        ZongMenBountyPlayerResponse_4000007d defaultInstance = ZongMenBountyPlayerResponse_4000007d.getDefaultInstance();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        ZongMenBountyPlayerResponse_4000007d.Builder resp = ZongMenBountyPlayerResponse_4000007d.newBuilder();
        client.sendProtocol(resp.build());
    }
    ...
    private void updateMemberFightPower(NetClient client, Object message) {
        ...
        long pid = req.getPid();
        int fightPower = req.getFightPower();
    ...
    private void bountyReward(NetClient client, Object message) {
        ZongMenBountyRewardRequest_40000072 req = (ZongMenBountyRewardRequest_40000072) message;
        ZongMenBountyRewardResponse_40000073 defaultInstance = ZongMenBountyRewardResponse_40000073.getDefaultInstance();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        ZongMenBountyRewardResponse_40000073.Builder resp = ZongMenBountyRewardResponse_40000073.newBuilder();
        client.sendProtocol(resp.build());
    }

    private void donate(NetClient client, Object message) {
        ZongMenDonateRequest_40000067 req = (ZongMenDonateRequest_40000067) message;
        int id = req.getId();
        ZongMenDonateResponse_40000068 defaultInstance = ZongMenDonateResponse_40000068.getDefaultInstance();
        ZongMenDonateResponse_40000068.Builder resp = ZongMenDonateResponse_40000068.newBuilder();
        Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
        GuildDonateConfig guildDonateConfig = GuildDonateManager.instance().get(id); 
        IntMapWrapper donateMap = player.getZongmenModule().getDonateMap(); 
