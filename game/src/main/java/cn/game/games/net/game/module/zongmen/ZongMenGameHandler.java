...
public class ZongMenGameHandler extends BaseHandler {
  ...
  private void applyJoinZongMen(NetClient client, Object o) {
    ...
    sendMsgToZongMenServer(player, req, player.getAttrModule().getPower()+"",player.getPlayerName())
        .onSuccess(
            callBack -> {
              if (callBack.errorCode != ErrorMsgEnum.ok.ID) {
                client.sendProtocol(res.build(), callBack.errorCode);
    ...
    checkFuture
        ...
        .thenCombine(
            ...
            (nameFail, nameRepeatCheck) -> {
              ...
              if (nameFail) {
                // 创建宗门
                sendMsgToZongMenServer(player, req, player.getPlayerName(), player.getAttrModule().getPower()+"")
                    .onSuccess(
