...
//            rpcFuture = VxHolder.requestRemoteServer("LY_ZONG_MEN", serverReq.build());
		log.info(String.format("sendMsgToZongMenServer pid:%d, msgId:%d, zongMenId:%d, req:%s", player.getPlayerId(), reqMsgId, zongMenId,
				req));
		rpcFuture.onSuccess( // 请求成功
				result -> {
					if (result != null) {
						log.info(String.format("sendMsgToZongMenServerCallBack msgId:%d %s, errorCode:%d, pid:%d ", result.getMsgId(),
								req.getClass().getSimpleName(), result.getErrorCode(), result.getPlayerId()));
	...
	private void getMyApplyZongMenIdList(NetClient client, Object o) {
		...
		ZongMenMsg.ZongMenGetMyApplyZongMenIdListResponse_40000056.Builder res = ZongMenMsg.ZongMenGetMyApplyZongMenIdListResponse_40000056
				.newBuilder();
		Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
		ZongMenModule zongMenModule = player.getZongmenModule();
	...
	private void quitZongMen(NetClient client, Object o) {
		...
		autoForwardZongMenServer(client, res, req, (result) -> { // 退出成功
			Player player = PlayerManager.getInstance().getPlayer(client.getPlayerId());
	...
	private void applyJoinZongMen(NetClient client, Object o) {
		...
		sendMsgToZongMenServer(req.getId(), player, req, player.getAttrModule().getPower() + "", player.getPlayerName())
				.onSuccess(callBack -> {
					if (callBack.errorCode != ErrorMsgEnum.ok.ID) {
						client.sendProtocol(res.build(), callBack.errorCode);
					} else {
	...
	private void createZongMen(NetClient client, Object o) {
		...
		CompletableFuture.allOf(checkComplatableList.toArray(new CompletableFuture[0])).thenAcceptAsync(result -> {
			...
			// 创建宗门
			sendMsgToZongMenServer(player, req, player.getPlayerName(), player.getAttrModule().getPower() + "", player.getServerId())
					.onSuccess(createZongMenCallback -> {
