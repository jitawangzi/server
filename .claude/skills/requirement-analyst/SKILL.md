...
*   **Config & Boundaries (配置与边界)**:
    *   **数值忽略 (Static Config)**: 凡是可以通过静态配置解决的数值问题（如：具体刷新时间点、消耗曲线数值、具体概率），**不算**作待确认问题。直接在规则中注明“由配置定义”。
    *   **服务端权威 (Server Authority Default)**: 凡涉及时间校验（限时）、资源扣除、状态限制，**默认**必须由服务端进行强校验。不要问“是否需要服务端校验”，这是必须的。
    *   **UI表现分离 (Client Presentation Independence)**: 凡涉及“解锁后即时刷新还是重开界面”、“红点如何消除”等纯表现层问题，若服务端数据状态已正确变更，则**不算**作待确认问题。默认假设客户端根据状态自行处理刷新。
    *   **默认边界处理**: 如果文档未说明达到上限后的具体行为，**默认**视为“操作失败，返回通用错误码”，**无需**提问。
...
## Interaction (交互指引)
1.  **Initial Delivery**: 任务完成后，如果在“F. 待确认问题”中有内容，请在对话中显式提示用户查阅或回复。
2.  **Feedback Loop (反馈循环)**: 如果用户针对生成的文档提出了修改意见或澄清了待确认问题，**必须**根据新的信息修正并**刷新**输出的 `01_server_rules.md` 文档。
