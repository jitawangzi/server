...
进入全自动闭环阶段，处理所有失败直至通过。

### 7.1 编译与启动
* AI 检查语法错误。
* 执行 `powershell .\run_server.ps1` 进行编译并启动服务器。

### 7.2 运行测试与故障分类 (Triage)
* AI (QA 角色) 执行 `powershell .\run_client.ps1 <TestClass>`。
* **如果测试失败**，QA 角色必须分析日志并输出 `09_test_run_report.md`：
    * **CASE A: TEST 故障** (测试代码写错了)。
        * **行动**：QA 角色调用 `rule-fix-failures.md` 自行修复测试代码，回到 7.2 重新运行。
    * **CASE B: IMPL 故障** (业务逻辑写错了)。
        * **行动**：QA 生成 **Detailed Bug Report**。移交给开发角色。
    * **CASE C: CONTRACT 故障** (设计有缺陷)。
        * **行动**：停机，请求人工干预或返回设计阶段。

### 7.3 开发修复 (Fix)
* AI (开发角色) 接收 Bug Report。
* 分析日志，定位代码，调用 `rule-fix-failures.md` 修复业务逻辑。
* 完成后回到 7.1 重新编译启动。

### 7.4 归档与定稿
* 当所有测试用例显示 `✅ TEST PASSED`。
* AI 确保代码与设计文档完全同步，清理临时文件。
