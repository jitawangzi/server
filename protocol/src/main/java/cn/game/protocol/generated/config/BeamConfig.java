 ...
 */
 public class BeamConfig {
	...
	public final int[] ShowParam;		

	public BeamConfig (Element element) throws Exception {
		...
		String ShowParamString = element.getAttribute("ShowParam"); // 表现类型1参数 多次伤害时间间隔  表现类型2参数 时间间隔
		if (ShowParamString != null && ShowParamString.length() > 0) {
			String[] ShowParamStrings = ShowParamString.split(";"); 
			int[] ShowParamTemp = new int[ShowParamStrings.length] ; 
			for (int i = 0; i < ShowParamStrings.length; i++) {
				int temp = Integer.parseInt(ShowParamStrings[i]);	
				ShowParamTemp[i] = temp;
			}
			ShowParam = ShowParamTemp ;			
		} else {
			ShowParam = new int[] {};
		}
	}
