 ...
 public class HeroFashionConfig {
	...
	public HeroFashionConfig (Element element) throws Exception {
		...
		if (FashionValveString != null && FashionValveString.length() > 0) {
			...
			int[][] FashionValve = new int[FashionValveStrings.length][] ; 
			for (int i = 0; i < FashionValveStrings.length; i++) {
				...
				FashionValve[i] = array;
				
			}
			this.FashionValve = FashionValve ;			
		} else {
			this.FashionValve = new int[][] {};
		}
		...
		if (FashionStarValveString != null && FashionStarValveString.length() > 0) {
			...
			int[] FashionStarValve = new int[FashionStarValveStrings.length] ; 
			for (int i = 0; i < FashionStarValveStrings.length; i++) {
				int temp = Integer.parseInt(FashionStarValveStrings[i]);
				FashionStarValve[i] = temp;
			}
			this.FashionStarValve = FashionStarValve ;			
		} else {
			this.FashionStarValve = new int[] {};
		}
		this.FashionstarConsumeId = Integer.parseInt(element.getAttribute("FashionstarConsumeId") == null || element.getAttribute("FashionstarConsumeId").length() == 0 ? "0"
		...
		if (FashionStarReplaceString != null && FashionStarReplaceString.length() > 0) {
			...
			int[] FashionStarReplace = new int[FashionStarReplaceStrings.length] ; 
			for (int i = 0; i < FashionStarReplaceStrings.length; i++) {
				int temp = Integer.parseInt(FashionStarReplaceStrings[i]);
				FashionStarReplace[i] = temp;
			}
			this.FashionStarReplace = FashionStarReplace ;			
		} else {
			this.FashionStarReplace = new int[] {};
		}
	}
